...
bool RegionHandler::GetRegionSpectralData(int region_id, int file_id, const AxisRange& z_range, std::string& coordinate, int stokes_index,
    ...
    if (_frames.at(file_id)->UseLoaderSpectralData(lc_region->shape()) && !_frames.at(file_id)->IsZarrLoader()) {
        ...
        if (initial_region_state.type == CARTA::RegionType::POINT) {
            ...
            auto get_stokes_profiles_data = [&](ProfilesMap& tmp_results, int tmp_stokes_index) {
                ...
                if (!_frames.at(file_id)->GetLoaderPointSpectralData(tmp_profile, z_range, tmp_stokes_index, point, progress)) {
                    return false;
                }
    ...
    // Reset lc_region for computed stokes (must recalculate per constituent stokes index)
    if (Stokes::IsComputed(stokes_index)) {
        lc_region = nullptr;
    }

    if (Stokes::IsComputed(stokes_index)) {
        // ── Sequential path for computed stokes ──────────────────────────────────
        // get_profiles_data must call GetImageRegion per constituent stokes; not safe
        // to parallelise across slabs, so keep the original adaptive while loop.
        size_t start_z(z_range.from), count(0), end_z(0), profile_start(0);
        int delta_z = INIT_DELTA_Z;
        int dt_target = TARGET_DELTA_TIME;
        ...
        while (progress < 1.0) {
            ...
            if (delta_z < 1) delta_z = 1;
            if (delta_z > (int)profile_size) delta_z = (int)profile_size;
