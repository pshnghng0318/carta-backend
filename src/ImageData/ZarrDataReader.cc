...
bool ZarrDataReader::ReadChannelSlice(casacore::Array<float>& buffer, const casacore::Slicer& section) {
    ...
    spdlog::debug("ZarrDataReader::ReadChannelSlice: start={}, stop={}, length={}", 
    ...
    // time is not supported in CARTA yet so always set to 0.
    ...
    const size_t bytes_per_chunk_stripe = static_cast<size_t>(chunk_shape_l) * static_cast<size_t>(chunk_shape_f) * static_cast<size_t>(height_y) * sizeof(float);
    const size_t effective_max_bytes = std::max(max_bytes, bytes_per_chunk_stripe);
    
    int cols_per_batch = static_cast<int>(effective_max_bytes / bytes_per_chunk_stripe * chunk_shape_l);
    if (cols_per_batch <= 0) {
        cols_per_batch = chunk_shape_l;
    }
    ...
    while (x_offset < width_x) {
        ...
        auto slice_result =
            _impl->store
            // l, u
            | tensorstore::Dims(3).ClosedInterval(start[0] + x_offset, start[0] + x_offset + batch_width - 1)
            // m, v
            | tensorstore::Dims(4).ClosedInterval(start[1], stop[1])
            // time, freq, pol
            | tensorstore::Dims(0, 1, 2).IndexSlice({time_idx, freq_idx, pol_idx});
        ...
        // Buffer is Fortran-order [X, Y]: X is fast dimension.
        // So (x, y) is at index x + y * width_x
        // The batch starts at (x_offset, 0), so the pointer offset is just x_offset.
        float* batch_dst = dst_ptr + x_offset;
        
        // Create SharedArray with Explicit Strides to map into the larger buffer
        // Shape: [batch_width, height_y]
        // Strides (bytes): 
        //   Dim 0 (X): sizeof(float)
        //   Dim 1 (Y): width_x * sizeof(float)
        std::array<tensorstore::Index, 2> batch_shape = {
            static_cast<tensorstore::Index>(batch_width),
            static_cast<tensorstore::Index>(height_y)
        };
        
        std::array<tensorstore::Index, 2> batch_byte_strides = {
            static_cast<tensorstore::Index>(sizeof(float)),
            static_cast<tensorstore::Index>(width_x * sizeof(float))
        };
        
        tensorstore::StridedLayout<2> batch_layout(batch_shape, batch_byte_strides);
        tensorstore::ArrayView<float, 2> batch_view(batch_dst, batch_layout);
